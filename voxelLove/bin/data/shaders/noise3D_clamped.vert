...
void main() {
  ...
  float noiseStrength = snoise(vCentered.xyz + vec3(normMouse.y * 5.));
  noiseStrength = ((noiseStrength + 1.) * 0.5) * 4.;
  
  vec4 pushed = scale(vCentered, vec3(noiseStrength));
  pushed = length(pushed) > (1. * 4.) ? pushed : scale(pushed, vec3(0.5));
  //pushed = min(pushed, normalize(vCentered) );

  //vec4 pushNorm = normalize(pushed);
  //pushed = clamp(pushed, pushNorm * -1, pushNorm);
